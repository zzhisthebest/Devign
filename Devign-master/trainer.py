...
def evaluate_loss(model, loss_function, num_batches, data_iter, cuda=False):
    ...
    with torch.no_grad():
        ...
        for _ in range(num_batches):
            ...
            predictions,_ = model(graph, cuda=True)
...
def evaluate_metrics(model, loss_function, num_batches, data_iter):
    ...
    with torch.no_grad():
        ...
        for _ in range(num_batches):
            ...
            _loss.append(batch_loss.detach().cpu().item())
            predictions = predictions.detach().cpu()
            if predictions.ndim == 2:#当张量predictions为二维时
                all_predictions.extend(np.argmax(predictions.numpy(), axis=-1).tolist())
            else:#这个是二分类
                all_predictions.extend(
                    predictions.ge(torch.ones(size=predictions.size()).fill_(0.5)).to(
...
def train(model, dataset, epoches, dev_every, loss_function, optimizer, save_path, log_every=50, max_patience=5):
    ...
    try:
        for step_count in range(max_steps):
            ...
            #print("type(targets)",type(targets))#<class 'torch.Tensor'>
            targets = targets.cuda()
            predictions,_ = model(graph, cuda=True)
            predictions=predictions.cuda()
            batch_loss = loss_function(predictions, targets)
            #print("targets.shape",targets.shape)#torch.Size([128])
            #print("predictions.shape",predictions.shape)#torch.Size([128])
            ...
            if step_count % dev_every == (dev_every - 1):
                ...
                if patience_counter == max_patience:
                    break
    # 当用户按下Ctrl + C（即键盘中断）时，会触发KeyboardInterrupt异常。相当于可以人为结束训练，后面的代码仍会执行。
    #也可以直接按终止一次，不要按2次，按2次后面的代码在按第2次后就不继续执行力，这很容易理解
